...
handle_config(User, Config, [{log_events, Events}, {api_key, ApiKey}, {last_sync_time, Time}]) ->
    ...
    {reply, #{value => JsonValue, rule_id => RuleID}, [
        {log_events, NextEvents}, {api_key, ApiKey}, {last_sync_time, Time}
    ]}.
...
unsent_events([HEvents | TEvents], ApiKey) ->
    ...
    Client = application:get_env(statsig, network, network),
    case Client:request(ApiKey, "rgstr", Input) of
        false ->
...
get_and_save_config_specs(ApiKey, Time) ->
    % allow the client to be swapped out in test envs, for instance
    Client = application:get_env(statsig, network, network),
    case Client:request(ApiKey, "download_config_specs", #{<<"sinceTime">> => Time}) of
        false -> Time;
