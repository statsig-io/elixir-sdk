...
eval_rules(User, [Rule | Rules], Config, Exposures) ->
    ...
    if
        RuleResult ->
            ...
            if
                Pass ->
                    {Rule, Pass, RuleJson, RuleID, AllExposures};
                true ->
                    {Rule, Pass, maps:get(<<"defaultValue">>, Config, #{}), RuleID, AllExposures}
            end;
        true ->
            eval_rules(User, Rules, Config, AllExposures)
    end.
    ...
    if
        length(Pass) > 0 ->
            {false, maps:get(<<"returnValue">>, Rule, []), maps:get(<<"id">>, Rule, <<"">>),
                Exposures};
        true ->
            {true, maps:get(<<"returnValue">>, Rule, []), maps:get(<<"id">>, Rule, <<"">>),
                Exposures}
    end.
...
compare(Value, Operator, Target) ->
    case Operator of
        ...
        <<"on">> ->
            ...
            (ValueYear == TargetYear) and
                (ValueMonth == TargetMonth) and
                (ValueDay == TargetDay);
        _ ->
...
version_compare_helper(Version1, Version2) ->
    ...
    if
        ...
        true ->
            compare_version_part(Parts1, Parts2)
    end.
...
trim(Val) ->
    ...
    if
        Rest == nomatch -> Str;
        length(Rest) /= length(Str) -> string:slice(Str, 0, length(Str) - length(Rest));
        true -> Str
    end.
...
get_evaluation_value(User, Condition) ->
    ...
    case Type of
        ...
        <<"unit_id">> ->
            {false, false, get_unit_id(User, IdType), []};
